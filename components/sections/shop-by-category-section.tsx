        ...
        <div className="mb-10 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="type-body mt-3 max-w-md">
              Browse handcrafted furniture by room and purpose.
            </p>
          </div>
          <Link href="/shop" className="btn-outline-sm self-start md:self-auto">
            View All
          </Link>
        </div>
        <div className="grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-5 md:gap-6">
          {categories.map((category) => (
            ...
            >
              ...
              <h3 className="type-h3 text-base">{category.name}</h3>
              <p className="type-price-sm mt-1 text-muted-foreground">{category.startingPrice}</p>
            </Link>
          ))}
        </div>
      </div>
